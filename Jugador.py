...
class Jugador():
    ...
    def mano(self, hand):
        ...
        suma_mano_inicial = [mazo.baraja[self.hand[0]],mazo.baraja[self.hand[1]]]            
        
        if (sum(suma_mano_inicial) > 21 and self.contiene_as()):
        ...
        if (vm.contiene_as(self.hand) == True and vm.contiene_figura(self.hand) == True):
            ...
            print(f"Cartas de {self.nombre_jugador} son: [{self.hand[0]}][{self.hand[1]}] y suman {sum(suma_mano_inicial)}")
